...
//*******************************************************
// HAL: DAKEFPV 2.4GHz TX, LR1121, ESP32
//*******************************************************
...
//-- UARTs
// UARTB = serial (USB-C, CRSF/MavLink)
// UARTC = COM (CLI, USB-C)
// UART  = JR bay pin5
// UARTD = serial2 (ESP32-C3 backpack)
// UARTE = in port (S.Bus)
...
void lr11xx_rfpower_calc(const int8_t power_dbm, int8_t *sx_power, int8_t *actual_power_dbm, const uint8_t frequency_band)
{
    if (frequency_band == SX_FHSS_FREQUENCY_BAND_2P4_GHZ) {
        if (power_dbm >= POWER_30_DBM)      { *sx_power = 2;   *actual_power_dbm = 30; }
        else if (power_dbm >= POWER_27_DBM) { *sx_power = 0;   *actual_power_dbm = 27; }
        else if (power_dbm >= POWER_24_DBM) { *sx_power = -2;  *actual_power_dbm = 24; }
        else if (power_dbm >= POWER_20_DBM) { *sx_power = -6;  *actual_power_dbm = 20; }
        else if (power_dbm >= POWER_17_DBM) { *sx_power = -8;  *actual_power_dbm = 17; }
        else if (power_dbm >= POWER_14_DBM) { *sx_power = -14; *actual_power_dbm = 14; }
        else                                { *sx_power = -18; *actual_power_dbm = 10; }
    } else {
        if (power_dbm >= POWER_30_DBM)      { *sx_power = 5;   *actual_power_dbm = 30; }
        else if (power_dbm >= POWER_27_DBM) { *sx_power = -3;  *actual_power_dbm = 27; }
        else if (power_dbm >= POWER_24_DBM) { *sx_power = -7;  *actual_power_dbm = 24; }
        else if (power_dbm >= POWER_20_DBM) { *sx_power = -11; *actual_power_dbm = 20; }
        else if (power_dbm >= POWER_17_DBM) { *sx_power = -14; *actual_power_dbm = 17; }
        else if (power_dbm >= POWER_14_DBM) { *sx_power = -16; *actual_power_dbm = 14; }
        else                                { *sx_power = -17; *actual_power_dbm = 10; }
    }
}
